...
<div class="row">
  <div class="col-md-12">
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('addSupervise1') }}"enctype="multipart/form-data">
          @csrf 
          @if ($errors->any())
      ...
      @endif
          <div class="form-row">
           ...
           <div class="row">
            <div class="form-group col-md-4">
              <label for="inputAddress">ชื่ออาจารย์</label>
       <input type="text" class="form-control" @error('name') is-invalid @enderror name="name" value="{{ old('name') }}"  autofocus placeholder="ชื่ออาจารย์">
          ...
          <div class="form-group col-md-4">
            <label for="inputAddress">หลักสูตร</label>
            {{-- <input type="text" class="form-control" @error('name') is-invalid @enderror name="name" value="{{ old('name') }}"  autofocus placeholder="name"> --}}
            <select class="form-control select2" id="validationSelect1" name="course" required>
              <option value="">กรุณาเลือกหลักสูตร</option>
              {{-- @foreach ($establishment as $row) --}}
              {{-- <optgroup label="Mountain Time Zone"> --}}
                {{-- <option value="{{$row->id}}">{{$row->address}}</option> --}}
                <option value="วิทยาศาสตรบัณฑิต สาขาวิชาเทคโนโลยีสารสนเทศ">วิทยาศาสตรบัณฑิต สาขาวิชาเทคโนโลยีสารสนเทศ</option>
                <option value="วิทยาศาสตรบัณฑิต สาขาวิชาวิทยาการคอมพิวเตอร์">วิทยาศาสตรบัณฑิต สาขาวิชาวิทยาการคอมพิวเตอร์</option>
      ...
      <br>
          <div class="modal-footer">
            <a href="/officer/Supervise" type="submit" class="btn mb-2 btn-success" >ย้อนกลับ</a>
